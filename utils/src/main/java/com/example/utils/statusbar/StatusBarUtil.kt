package com.example.utils.statusbar
...
object StatusBarUtil {
     ...
     * @param dark：true/false：状态栏字体的颜色为黑色/状态栏字体的颜色为白色
     */
    fun setStatusBarDarkTheme(activity: Activity, dark: Boolean): Boolean {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            when {
                ...
                OSUtils.isMiui -> setStatusBarFontIconDark(
                    activity,
                    TYPE_MIUI,
                    dark
                )
                OSUtils.isFlyme -> setStatusBarFontIconDark(
                    activity,
                    TYPE_FLYME,
                    dark
                )
